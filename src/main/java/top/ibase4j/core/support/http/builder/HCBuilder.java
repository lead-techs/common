package top.ibase4j.core.support.http.builder;
...
import top.ibase4j.core.exception.HttpProcessException;
import top.ibase4j.core.support.http.common.SSLs;
import top.ibase4j.core.support.http.common.SSLs.SSLProtocolVersion;
...
public class HCBuilder extends HttpClientBuilder {
    ...
    /**
     * 设置超时时间以及是否允许网页重定向（自动跳转 ）
     *
     * @param timeout		超时时间，单位-毫秒
     * @param redirectEnable		自动跳转
     * @return	返回当前对象
     */
    public HCBuilder timeout(int timeout,  boolean redirectEnable){
        // 配置请求的超时设置
     ...
     */
    public HCBuilder ssl() throws HttpProcessException {
...
//		return (HCB) this.setConnectionManager(connManager);
        return (HCBuilder) this.setSSLSocketFactory(ssls.getSSLCONNSF(sslpv));
    }
     ...
     * @throws HttpProcessException	http处理异常
     */
    public HCBuilder ssl(String keyStorePath) throws HttpProcessException{
     ...
     * @throws HttpProcessException	http处理异常
     */
    public HCBuilder ssl(String keyStorePath, String keyStorepass) throws HttpProcessException{
...
//		this.isNewSSL=true;
     ...
     * @throws HttpProcessException	http处理异常
     */
    public HCBuilder pool(int maxTotal, int defaultMaxPerRoute) throws HttpProcessException{
        Registry<ConnectionSocketFactory> socketFactoryRegistry = RegistryBuilder
                .<ConnectionSocketFactory> create()
                .register("http", PlainConnectionSocketFactory.INSTANCE)
        ...
        isSetPool=true;
        return (HCBuilder) this.setConnectionManager(connManager);
    }
     ...
     * @return	返回当前对象
     */
    public HCBuilder proxy(String hostOrIP, int port){
        // 依次是代理地址，代理端口号，协议类型
        HttpHost proxy = new HttpHost(hostOrIP, port, "http");
        DefaultProxyRoutePlanner routePlanner = new DefaultProxyRoutePlanner(proxy);
        return (HCBuilder) this.setRoutePlanner(routePlanner);
    }
     ...
     * @return	返回当前对象
     */
    public HCBuilder retry(final int tryTimes){
     ...
     * @return	返回当前对象
     */
    public HCBuilder retry(final int tryTimes, final boolean retryWhenInterruptedIO){
        // 请求重试处理
     ...
     * @return	返回当前对象
     */
    public HCBuilder sslpv(String sslpv){
     ...
     * @return	返回当前对象
     */
    public HCBuilder sslpv(SSLProtocolVersion sslpv){
        this.sslpv = sslpv;
        return this;
    }
}
